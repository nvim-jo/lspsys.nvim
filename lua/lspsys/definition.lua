...
function def:close_all()
  ...
  api.nvim_del_augroup_by_name('SysPeekdefinition')
end
  ...
  for action, map in pairs(config.definition.keys) do
    if action ~= 'close' then
      util.map_keys(bufnr, map, function()
        ...
        if not ok then
          api.nvim_err_writeln('[Lspsys] jump failed on definition')
          return
        end
...
function def:clean_event()
  api.nvim_create_autocmd('WinClosed', {
    group = api.nvim_create_augroup('SysPeekdefinition', { clear = true }),
    ...
    desc = '[lspsys] peek definition clean data event',
  })
end
...
function def:peek_definition(method)
  if self.pending_reqeust then
    vim.notify(
      '[lspsys] a peek_definition request has already been sent, please wait.',
      vim.log.levels.WARN
    )
    return
  end
  ...
  lsp.buf_request(current_buf, method_name, params, function(_, result, context)
    ...
    if not result or next(result) == nil then
      if #self.list == 0 and count == 0 then
        vim.notify(
          '[lspsys] response of request method ' .. method_name .. ' is empty',
          vim.log.levels.WARN
        )
      end
      return
