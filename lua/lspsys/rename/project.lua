...
local ns = api.nvim_create_namespace('SysProjectRename')
...
local function apply_map(bufnr, winid, data, new_name)
  util.map_keys(bufnr, config.rename.keys.select, function()
    ...
    if not item.selected then
      item.selected = true
      api.nvim_buf_add_highlight(bufnr, ns, 'SysSelect', curlnum - 1, 0, -1)
      return
    end
    item.selected = false
...
local function render(chunks, new_name)
  ...
  for fname, item in pairs(result) do
    ...
    api.nvim_buf_add_highlight(bufnr, ns, 'SysFinderFname', line - 1, 0, -1)
    line = line + 1
...
function M:new(args)
  if fn.executable('rg') == 0 then
    vim.notify('[lspsys] failed finding rg')
    return
  end

  if #args < 2 then
    vim.notify('[lspsys] missing search pattern or new name')
    return
  end
  ...
  if not root_dir then
    vim.notify('[lspsys] buffer run in single file mode')
    return
  end
