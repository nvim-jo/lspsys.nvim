...
function hover:open_floating_preview(content, option_fn)
  ...
  if not option_fn then
    api.nvim_create_autocmd({ 'CursorMoved', 'InsertEnter', 'BufDelete' }, {
      ...
      desc = '[Lspsys] Auto close hover window',
    })
...
function hover:do_request(args)
  ...
  if #clients == 0 then
    ...
    vim.notify('[lspsys] hover is not supported by the servers of the current buffer')
    return
  end
  local count = 0
...
function hover:render_hover_doc(args)
  local diag_winid = require('lspsys.diagnostic').winid
  if diag_winid and api.nvim_win_is_valid(diag_winid) then
    require('lspsys.diagnostic'):clean_data()
  end
  args = args or {}

  if not check_parser() then
    vim.notify(
      '[lspsys] please install markdown and markdown_inline parser in nvim-treesitter',
      vim.log.levels.WARN
    )
    return
  end
  ...
  if self.pending_request then
    print('[lspsys] a hover request has already been sent, please wait.')
    return
  end
