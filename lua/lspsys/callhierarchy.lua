...
local ns = api.nvim_create_namespace('SysCallhierarchy')
    ...
    timer:start(
      ...
      vim.schedule_wrap(function()
        ...
        buf_set_extmark(self.left_bufnr, ns, node.value.winline - 1, col, {
          ...
          virt_text = { { spinner[frame], 'SysSpinner' } },
...
function ch:set_data_icon(curlnum, data, col)
  buf_set_extmark(self.left_bufnr, ns, curlnum, col, {
    virt_text = { { kind[data.kind][2], 'Sys' .. kind[data.kind][3] } },
    virt_text_pos = 'overlay',
  })
end
...
function ch:toggle_or_request()
  if self.pending_request then
    vim.notify(
      ('[lspsys] a request for %s has already been sent, please wait.'):format(self.method),
      vim.log.levels.WARN
    )
    return
  end
...
function ch:peek_view()
  api.nvim_create_autocmd('CursorMoved', {
    group = api.nvim_create_augroup('SysCallhierarchy', { clear = true }),
    ...
    callback = function()
      ...
      api.nvim_set_option_value('winhl', 'Normal:SysNormal,FloatBorder:SysBorder', {
        scope = 'local',
    ...
    desc = '[Lspsys] callhierarchy peek preview',
  })
end
...
function ch:render_virtline(row, inlevel)
  for i = 1, inlevel - 4, 2 do
    ...
    if i + 2 > inlevel - 4 then
      virt = {
        { config.ui.lines[2], 'SysVirtLine' },
        { config.ui.lines[4], 'SysVirtLine' },
      }
    else
      virt = {
        { config.ui.lines[3], 'SysVirtLine' },
      }
    end
...
function ch:call_hierarchy(item, client, timer_close, curlnum)
  ...
  client.request(self.method, { item = item }, function(_, res)
    ...
    if not res or vim.tbl_isempty(res) then
      vim.notify('[lspsys] callhierarchy result is empty', vim.log.levels.WARN)
      return
    end
...
function ch:send_prepare_call()
  if self.pending_request then
    vim.notify('[lspsys] a request has already been sent, please wait.')
    return
  end
  self.main_buf = api.nvim_get_current_buf()
  local clients = util.get_client_by_method(get_method(1))
  if #clients == 0 then
    vim.notify('[lspsys] callhierarchy is not supported by the clients of the current buffer')
    return
  end
  local client
  if #clients == 1 then
    ...
    if choice == 0 or choice > #clients then
      api.nvim_err_writeln('[Lspsys] wrong choice for select client')
      return
    end
