...
local preview = require('lspsys.codeaction.preview')
...
function act:code_action(options)
  if self.pending_request then
    vim.notify(
      '[lspsys] a code action has already been requested, please wait.',
      vim.log.levels.WARN
    )
    return
  end
  ...
  if config.code_action.only_in_cursor and not options.context then
    options.context = {
      diagnostics = require('lspsys.diagnostic'):get_cursor_diagnostic(),
    }
  end
