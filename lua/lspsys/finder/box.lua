...
function M.filter(method, results)
  ...
  if type(fn) ~= 'function' then
    vim.notify('[lspsys] filter must be a function', vim.log.levels.ERROR)
    return
  end
  local retval = {}
...
function M.spinner()
  ...
  timer:start(0, 50, function()
    vim.schedule(function()
      ...
      api.nvim_buf_add_highlight(bufnr, 0, 'SysSpinner', 0, 0, -1)
...
local function to_normal_bg()
  local data = api.nvim_get_hl_by_name('SysNormal', true)
  if data.background then
    return { fg = data.background }
  end
  return { link = 'SysVirtLine' }
end
...
function M.indent_current(inlevel)
  ...
  if current == 4 then
    api.nvim_buf_set_extmark(0, con_ns, currow, current + 1, {
      virt_text = { { config.ui.lines[4], 'SysInCurrent' } },
  ...
  for i = 0, api.nvim_buf_line_count(0) - 1 do
    vim.tbl_map(function(item)
      local hi = (item == current and i >= range[1] and i <= range[2])
          and { link = 'SysInCurrent' }
        or to_normal_bg()
      api.nvim_set_hl(0, 'SysIndent' .. i .. item, hi)
    end, t)
  end
end
  ...
  api.nvim_set_decoration_provider(ns, {
    ...
    on_line = function(_, winid, bufnr, row)
      ...
      for i = 1, total, 2 do
        ...
        api.nvim_set_hl(0, hi, { link = 'SysNormal', default = true })
      end
    end,
  })
end
