...
local box = require('lspsys.finder.box')
...
local select_ns = api.nvim_create_namespace('SysSelect')
...
local ns = api.nvim_create_namespace('SysFinder')

function fd:init_layout()
  local win_width = api.nvim_win_get_width(0)
  if config.finder.right_width > 0.6 then
    vim.notify('[lspsys] finder right width must be less than 0.7')
    ...
    for i, res in ipairs(item.result or {}) do
      ...
      if i == 1 then
        ...
        buf_set_extmark(self.lbufnr, ns, row, 0, {
          virt_text = { { ' ' .. vim.tbl_count(item.result) .. ' ', 'SysCount' } },
          virt_text_pos = 'eol',
        })
        row = row + 1
      end
...
function fd:new(args)
  ...
  if #methods == 0 then
    vim.notify(
      ('[lspsys] no servers of buffer %s makes these methods available %s'):format(
        curbuf,
        table.concat(args, ' ')
      ),
      vim.log.levels.WARN
  ...
  coroutine.resume(coroutine.create(function()
    ...
    if not self.lwinid then
      spin_close()
      vim.notify('[Lspsys] finder no any results to show', vim.log.levels.WARN)
    end
  end))
end
