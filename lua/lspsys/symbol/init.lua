...
function symbol:register_module()
  local group = api.nvim_create_augroup('LspsysSymbols', { clear = true })
  ...
  api.nvim_create_autocmd('LspDetach', {
    ...
    callback = function(args)
      if self[args.buf] then
        self[args.buf] = nil
        if config.symbol_in_winbar.enable then
          pcall(api.nvim_del_augroup_by_name, 'SysWinbar' .. args.buf)
        end
      end
    end,
  })
...
function symbol:outline()
  require('lspsys.symbol.outline'):outline()
end
