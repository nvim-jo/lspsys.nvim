...
local config = require('lspsys').config
local util = require('lspsys.util')
local symbol = require('lspsys.symbol')
local win = require('lspsys.window')
...
local ns = api.nvim_create_namespace('SysOutline')
local beacon = require('lspsys.beacon').jump_beacon
local slist = require('lspsys.slist')
local ly = require('lspsys.layout')
local ctx = {}
  ...
  local function recursive_parse(data, level)
    for i, node in ipairs(data) do
      ...
      buf_set_extmark(self.bufnr, ns, row - 1, #indent - 2, {
        virt_text = { { kind[node.kind][2], 'Sys' .. kind[node.kind][1] } },
        virt_text_pos = 'overlay',
      })
      local inlevel = 4 + 2 * level
      if inlevel == 4 and not node.children then
        local virt = {
          { row == 1 and config.ui.lines[5] or config.ui.lines[2], 'SysVirtLine' },
          { config.ui.lines[4]:rep(2), 'SysVirtLine' },
        }
        ...
        for j = 1, inlevel - 4, 2 do
          ...
          if (not node.children or #node.children == 0) and j + 2 > inlevel - 4 then
            virt[#virt + 1] = i == #data and { config.ui.lines[1], 'SysVirtLine' }
              or { config.ui.lines[2], 'SysVirtLine' }
            virt[#virt + 1] = { config.ui.lines[4]:rep(2), 'SysVirtLine' }
          else
            virt = { { config.ui.lines[3], 'SysVirtLine' } }
          end
      ...
      if config.outline.detail then
        buf_set_extmark(self.bufnr, ns, row - 1, 0, {
          virt_text = { { node.detail or '', 'SysDetail' } },
        })
      end
      ...
      if node.children and #node.children > 0 then
        ...
        buf_set_extmark(self.bufnr, ns, row - 1, #indent - 4, {
          id = copy.virtid,
          virt_text = { { config.ui.collapse, 'SysToggle' } },
          virt_text_pos = 'overlay',
        })
...
function ot:collapse(node, curlnum)
  ...
  while tmp do
    ...
    buf_set_extmark(self.bufnr, ns, row, level - 2, {
      virt_text = { { icon, 'Sys' .. kind[tmp.value.kind][1] } },
      virt_text_pos = 'overlay',
    })
    ...
    for j = 1, level - 4, 2 do
      ...
      if j + 2 > level - 4 and not has_child then
        virt[#virt + 1] = islast and { config.ui.lines[1], 'SysVirtLine' }
          or { config.ui.lines[2], 'SysVirtLine' }
        virt[#virt + 1] = { config.ui.lines[4]:rep(2), 'SysVirtLine' }
      else
        virt = { { config.ui.lines[3], 'SysVirtLine' } }
    ...
    if config.outline.detail then
      buf_set_extmark(self.bufnr, ns, row, 0, {
        virt_text = { { tmp.value.detail or ' ', 'SysDetail' } },
      })
    end
...
function ot:toggle_or_jump()
  ...
  if node.value.expand then
    ...
    buf_set_extmark(self.bufnr, ns, curlnum - 1, inlevel - 4, {
      ...
      virt_text = { { config.ui.expand, 'SysToggle' } },
      virt_text_pos = 'overlay',
    })
...
function ot:create_preview_win(lines)
  ...
  self.preview_bufnr, self.preview_winid = win
    ...
    :winopt({
      ['winhl'] = 'NormalFloat:SysNormal,FloatBorder:SysBorder',
      ['sidescrolloff'] = 5,
    })
    :wininfo()
end
...
function ot:auto_close(group)
  api.nvim_create_autocmd('WinEnter', {
    ...
    desc = '[Lspsys] auto close the outline window when is last',
  })
end
...
function ot:outline(buf)
  ...
  local res = not util.nvim_ten() and symbol:get_buf_symbols(buf)
    or require('lspsys.symbol.head'):get_buf_symbols(buf)

  if not res or not res.symbols or #res.symbols == 0 then
    vim.notify(
      '[lspsys] failed finding symbols - server may not be initialized, try again later.',
      vim.log.levels.INFO
    )
    return
  end
