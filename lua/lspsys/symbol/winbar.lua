local api = vim.api
local config = require('lspsys').config.symbol_in_winbar
local ui = require('lspsys').config.ui
...
local function bar_prefix()
  return {
    prefix = '%#Sys',
    sep = '%#SysWinbarSep#' .. config.separator .. '%*',
  }
end
...
local function path_in_bar(buf)
  ...
  for item in util.path_itera(buf) do
    item = #items == 0
        and '%#' .. (hl or 'SysFileIcon') .. '#' .. (icon and icon .. ' ' or '') .. '%*' .. bar.prefix .. 'FileName#' .. item .. '%*'
...
local function init_winbar(buf)
  ...
  api.nvim_create_autocmd('User', {
    ...
    desc = 'Lspsys get and show symbols',
  })

  api.nvim_create_autocmd({ 'CursorMoved' }, {
    group = api.nvim_create_augroup('SysWinbar' .. buf, { clear = true }),
    buffer = buf,
    callback = function(args)
      local res = not util.nvim_ten() and symbol:get_buf_symbols(args.buf)
        or require('lspsys.symbol.head'):get_buf_symbols(args.buf)
      if res and res.symbols then
        render_symbol_winbar(args.buf, res.symbols)
      end
    end,
    desc = 'Lspsys symbols render and request',
  })
end
...
local function get_bar()
  ...
  local res = not util.nvim_ten() and symbol:get_buf_symbols(curbuf)
    or require('lspsys.symbol.head'):get_buf_symbols(curbuf)
  if res and res.symbols then
