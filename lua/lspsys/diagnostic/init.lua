...
local act = require('lspsys.codeaction')
...
local jump_beacon = require('lspsys.beacon').jump_beacon
local nvim_buf_del_keymap = api.nvim_buf_del_keymap
local action_preview = require('lspsys.codeaction.preview').action_preview
local preview_win_close = require('lspsys.codeaction.preview').preview_win_close
...
function diag:code_action_cb(action_tuples, enriched_ctx)
  ...
  for index, client_with_actions in pairs(action_tuples) do
    if #client_with_actions ~= 2 then
      vim.notify('[lspsys] failed indexing client actions')
      return
    end
  ...
  api.nvim_buf_add_highlight(self.bufnr, 0, 'SysTitle', start_line, #config.ui.actionfix, -1)
  ...
  api.nvim_create_autocmd('CursorMoved', {
    ...
    desc = 'Lspsys show code action preview in diagnostic window',
  })
  ...
  local function scroll_with_preview(direction)
    api.nvim_win_call(self.winid, function()
      ...
      if curlnum >= sline then
        api.nvim_buf_add_highlight(self.bufnr, ns, 'SysSelect', curlnum - 1, 6, -1)
      end
